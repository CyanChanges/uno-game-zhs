...
describe('Slot allocation', () => {
  ...
  it('typed names are isolated between concurrent tabs', { timeout: 15000 }, async () => {
    ...
    await typeName(a, '11'); await typeName(b, '22'); await typeName(c, '33')
  ...
  it('sequential close-reopen: 11/22/33 in slot order', { timeout: 30000 }, async () => {
    ...
    const result = await Promise.all(r2.map(async p => ({ slot: await getSlot(p), name: await getName(p) })))
  ...
  it('parallel close-reopen: each slot keeps its own name', { timeout: 30000 }, async () => {
    ...
    const r2 = await Promise.all(NAMES.map(async () => {
      const p = await ctx.newPage()
    ...
    const result = await Promise.all(r2.map(async ({ p }) => ({
      slot: await getSlot(p), name: await getName(p),
    })))
  ...
  it('after the dust settles, no two tabs hold the same slot', { timeout: 60000 }, async () => {
    ...
    for (let round = 0; round < 3; round++) {
      const tabs = await Promise.all([0, 1, 2, 3].map(async () => {
        const p = await ctx.newPage()
        const navP = p.goto(BASE)
        return { p, navP }
      }))
  ...
  it('collision is resolved within a few heartbeats even in restored state', { timeout: 30000 }, async () => {
  ...
  it('after closing all tabs and reopening one, it lands on slot 1', { timeout: 30000 }, async () => {
    const ctx = await browser.newContext()
    const r1 = []
    for (const n of ['', '22']) {
      const p = await openTab(ctx)
  ...
  it('F5 within an active session keeps the same slot via stored identity', { timeout: 15000 }, async () => {
    const ctx = await browser.newContext()
    const p = await openTab(ctx)
  ...
  it('opening a fresh tab in a recycled slot does not inherit a saved playerId', { timeout: 30000 }, async () => {
    const ctx = await browser.newContext()
    const p = await openTab(ctx)
  ...
  it('two parallel fresh tabs (no sessionStorage) end up on distinct slots', { timeout: 30000 }, async () => {
    ...
    await Promise.all([
      a.waitForFunction(() => Number(sessionStorage.getItem('unoSlot')) > 0, { timeout: 5000 }),
      b.waitForFunction(() => Number(sessionStorage.getItem('unoSlot')) > 0, { timeout: 5000 }),
    ])
    await waitForStableSlots([a, b])
    const slots = [await getSlot(a), await getSlot(b)]
    expect(new Set(slots).size).toBe(2)
  ...
  it('two parallel restored tabs with the SAME stored slot end on distinct slots', { timeout: 30000 }, async () => {
  ...
  it('a closed tab with leftover unoPlayerId does not reserve the slot', { timeout: 30000 }, async () => {
    const ctx = await browser.newContext()
    const a = await openTab(ctx)
    expect(await getSlot(a)).toBe(1)
  ...
  it('original tab can F5-restore into its slot keeping its identity', { timeout: 30000 }, async () => {
    const ctx = await browser.newContext()
    const a = await openTab(ctx)
    const b = await openTab(ctx)
    expect(await getSlot(b)).toBe(2)
    ...
    await b.reload()
    await b.waitForFunction(() => Number(sessionStorage.getItem('unoSlot')) > 0, { timeout: 5000 })
    expect(await getSlot(b)).toBe(2)
