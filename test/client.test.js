...
describe("UNO Client", () => {
  ...
  it("shows (已准备) after player readies and state persists after opponent disconnects", async () => {
    ...
    await pageA.waitForFunction(
      () => {
        const items = document.querySelectorAll("#players li");
        return (
          items.length === 2 && items[1].classList.contains("disconnected")
        );
      },
      { timeout: 10000 },
    );
  ...
  it(
    "draw increases card count during normal play",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      const getCardCount = async (page) =>
        (await page.$$("#player-hand .card")).length;
    ...
    async () => {
      ...
      const initialDisplay = await pageA.evaluate(() => {
        const cards = Array.from(
          document.querySelectorAll("#opponent-hands .player"),
        );
      ...
      const bobId = await pageB.evaluate(() =>
        sessionStorage.getItem("unoPlayerId"),
      );
      await pageB.close();
  ...
  it("name and lobby ID saved to localStorage on input", async () => {
    ...
    const name1 = await page.evaluate(
      () =>
        sessionStorage.getItem("unoPlayerName") ||
    ...
    const lobby1 = await page.evaluate(
      () =>
        sessionStorage.getItem("unoLobbyId") ||
        localStorage.getItem("unoLobbyId"),
    );
  ...
  it(
    "turn order display shows after game starts",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
  ...
  it(
    "3-player room: surrender removes player, game continues",
    { timeout: 45000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      await pageA.waitForFunction(
        () => document.querySelectorAll("#players li").length === 3,
      );
      await pageB.waitForFunction(
        () => document.querySelectorAll("#players li").length === 3,
      );
  ...
  it(
    "3 tabs join same lobby and see each other",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
  ...
  it(
    "spectator mode: join started lobby, watch game",
    { timeout: 45000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      const isSpectator = await pageB.evaluate(() =>
  ...
  it(
    "non-matching cards have no hover lift, matching cards do",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      await pageA.goto(BASE);
      ...
      const result = await pageA.evaluate((top) => {
        ...
        for (let i = 0; i < cards.length; i++) {
          ...
          if (isWild) {
            wildCount++;
            if (hasNotPlayable)
              return { error: "wild card should not be not-playable" };
          } else if (matches) {
            matchingCount++;
            if (hasNotPlayable)
              return { error: "matching card should not be not-playable" };
          } else {
            nonMatchingCount++;
            if (!hasNotPlayable)
              return { error: "non-matching card should be not-playable" };
          }
        }
  ...
  it(
    "invite AI button shows after leaving and re-creating lobby",
    { timeout: 30000 },
    async () => {
      const page = await browser.newPage();
      await page.goto(BASE);
  ...
  it(
    "drawing chain confirm dialog shows when breaking chain",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      const modalMsg = await pageB.$eval(
        "#modal-message",
        (el) => el.textContent,
      );
      ...
      const drawModalMsg = await pageB.$eval(
        "#modal-message",
        (el) => el.textContent,
      );
  ...
  it(
    "draw in drawing state accepts penalty and adds cards",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
  ...
  it(
    "new tab does not auto-join previous tab lobby",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      await pageA.goto(BASE);
  ...
  it(
    "surrender with 2 humans + AI does not end game",
    { timeout: 45000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
  ...
  it(
    "joining AI-only lobby does not show spectate offer",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      await pageA.goto(BASE);
  ...
  it(
    "wild color picker scrolls into view when shown",
    { timeout: 30000 },
    async () => {
      const page = await browser.newPage();
      await page.goto(BASE);
  ...
  it(
    "draw mode toggle visible to creator, not to others",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      const tooltip = await pageA.$eval("#draw-mode-info", (el) =>
  ...
  it(
    ...
    async () => {
      ...
      const slotA = await a.evaluate(() =>
        Number(sessionStorage.getItem("unoSlot")),
      );
      const slotB = await b.evaluate(() =>
        Number(sessionStorage.getItem("unoSlot")),
      );
      expect(slotA).toBe(1);
      expect(slotB).toBe(2);
      ...
      const slot = await lone.evaluate(() =>
        Number(sessionStorage.getItem("unoSlot")),
      );
      expect(slot).toBe(1);
  ...
  it(
    ...
    async () => {
      ...
      for (const p of round2)
        restored.push(await p.locator("#name").inputValue());
  ...
  it(
    ...
    async () => {
      ...
      const slotByName = {};
      for (let i = 0; i < 3; i++) {
        const slot = await round1[i].evaluate(() =>
          Number(sessionStorage.getItem("unoSlot")),
        );
  ...
  it(
    ...
    async () => {
      ...
      const slotA = await tabA.evaluate(() =>
        Number(sessionStorage.getItem("unoSlot")),
      );
      ...
      const slotB = await tabB.evaluate(() =>
        Number(sessionStorage.getItem("unoSlot")),
      );
  ...
  it(
    ...
    async () => {
      ...
      await turnPage.evaluate(
        (c) =>
          sendMessage({ action: "play", card: { color: c, type: "draw2" } }),
        top.color,
      );
  ...
  it(
    "modal Enter confirms, Escape cancels, Tab cycles focus",
    { timeout: 20000 },
    async () => {
      const page = await browser.newPage();
      await page.goto(BASE);
      ...
      const stillInLobby2 = await page.evaluate(
  ...
  // value drops.
  it(
    "turn timer counts down on the active turn",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
  ...
  it(
    "keyboard digit selects card and Enter plays it",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      await turnPage.waitForFunction(
        () => {
          const cards = document.querySelectorAll("#player-hand .card");
          return (
            cards.length > 0 && cards[0].classList.contains("keyboard-hover")
          );
        },
        { timeout: 3000 },
      );
      ...
      await turnPage.evaluate(
        (color) => sendMessage({ action: "dev_clear_hand" }),
        top.color,
      );
      await turnPage.waitForTimeout(200);
      await turnPage.evaluate(
        (color) =>
          sendMessage({ action: "dev_give_card", card: { color, type: "5" } }),
        top.color,
      );
      await turnPage.waitForFunction(() => {
        const cards = document.querySelectorAll("#player-hand .card");
  ...
  it(
    ...
    async () => {
      ...
      const activeBadges = await turnPage.$$eval(
  ...
  it(
    ...
    async () => {
      ...
      await turnPage.waitForFunction(
        () => {
          const el = document.querySelector(
            '#player-hand .card[data-test-id="card-zero"]',
          );
      ...
      const stillTagged = await turnPage.$(
        '#player-hand .card[data-test-id="card-zero"]',
      );
  ...
  it(
    "wild color picker plays a close animation",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      await turnPage.waitForTimeout(200);
      await turnPage.evaluate(() =>
        sendMessage({ action: "dev_give_card", card: { type: "wild" } }),
      );
      await turnPage.waitForFunction(
  ...
  it(
    "info icon shows a multi-line tooltip on hover/focus",
    { timeout: 20000 },
    async () => {
      const page = await browser.newPage();
      await page.goto(BASE);
  ...
  it(
    "wild color picker supports digit / arrow / Enter / Esc",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      await turnPage.waitForFunction(
        () => {
          ...
          return (
            card.getAttribute("data-color") === "green" &&
            card.getAttribute("data-type") === "wild"
          );
        },
        { timeout: 5000 },
      );
  ...
  it(
    "pressing the same digit twice plays the hovered card",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
  ...
  it(
    "sequential reopen restores names in 11/22/33 order",
    { timeout: 30000 },
    async () => {
      const ctx = await browser.newContext();
      const NAMES = ["11", "22", "33"];
  ...
  it(
    "rapid-fire reopen still restores names in slot order",
    { timeout: 30000 },
    async () => {
      const ctx = await browser.newContext();
      const NAMES = ["11", "22", "33"];
  ...
  it(
    "non-creator sees the draw-mode area as read-only",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      const creatorReadonly = await pageA.evaluate(() => {
        const el = document.getElementById("draw-mode-area");
        return (
          el && el.style.display !== "none" && el.classList.contains("readonly")
        );
      });
  ...
  it(
    "turn-order cards show name + count and mark self",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
      ...
      const layout = await pageA.evaluate(() => {
        ...
        const arrows = Array.from(
          document.querySelectorAll("#opponent-hands .order-arrow"),
        );
        return {
          cardCount: cards.length,
          arrowCount: arrows.length,
          selfCount: cards.filter((c) => c.classList.contains("self")).length,
          cardsHaveCount: cards.every((c) =>
            /\d+\s*张牌/.test(c.textContent || ""),
          ),
  ...
  it(
    "spectator state clears after game-over → rejoin",
    { timeout: 30000 },
    async () => {
      const page = await browser.newPage();
      await page.goto(BASE);
  ...
  it("reaction history records sent emojis", { timeout: 30000 }, async () => {
    ...
    await pageA.waitForFunction(
      () => {
        const rows = document.querySelectorAll(
          "#reaction-history .reaction-history-row",
        );
    ...
    const aSelf = await pageA.evaluate(() => {
      ...
      return (
        rows.length > 0 && rows[rows.length - 1].classList.contains("self")
      );
    });
  ...
  it(
    "off-turn not-playable cards do not double-dim",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
  ...
  it(
    "discard pile stays at full opacity off-turn",
    { timeout: 30000 },
    async () => {
      const pageA = await browser.newPage();
      const pageB = await browser.newPage();
  ...
  it(
    "lobby: Enter in the name input clicks join",
    { timeout: 30000 },
    async () => {
      const ctx = await browser.newContext();
  ...
  it(
    "lobby: pressing R toggles the ready button",
    { timeout: 30000 },
    async () => {
  ...
  it(
    "lobby: pressing I invites an AI for the creator",
    { timeout: 30000 },
    async () => {
      const ctx = await browser.newContext();
      const page = await ctx.newPage();
  ...
  it(
    "in-game: pressing D draws a card on your turn",
    { timeout: 30000 },
    async () => {
      const ctxA = await browser.newContext();
      const ctxB = await browser.newContext();
      ...
      const beforeHand = await turnPage.$$eval(
        "#player-hand .card",
        (els) => els.length,
      );
      await turnPage.evaluate(() => {
  ...
  it(
    "in-game: pressing S opens the surrender confirm",
    { timeout: 30000 },
    async () => {
      const ctxA = await browser.newContext();
      const ctxB = await browser.newContext();
  ...
  it(
    "Enter on the game-over overlay returns to lobby",
    { timeout: 30000 },
    async () => {
      const ctx = await browser.newContext();
      const page = await ctx.newPage();
  ...
  it(
    "in-game: pressing T toggles card layout",
    { timeout: 30000 },
    async () => {
      const ctxA = await browser.newContext();
      const ctxB = await browser.newContext();
  ...
  it(
    "off-turn hand cards have default cursor (not disabled)",
    { timeout: 30000 },
    async () => {
      const ctxA = await browser.newContext();
      const ctxB = await browser.newContext();
      ...
      await offPage.waitForFunction(
        () => {
          const card = document.querySelector("#player-hand .card");
          return (
            !!card && document.body.classList.contains("player-action-disabled")
          );
        },
        { timeout: 5000 },
      );
    ...
    async () => {
      ...
      const state = await pageA.evaluate(() => {
        ...
        return {
          ...
          modeHasChainClass: mode
            ? mode.classList.contains("mode-chain")
            : false,
  ...
  it(
    "chain pill shows the pending count in chain mode",
    { timeout: 30000 },
    async () => {
      const ctxA = await browser.newContext();
      const ctxB = await browser.newContext();
      ...
      const turnPage = (await isMyTurn(pageA)) ? pageA : pageB;
      await turnPage.evaluate(() =>
        sendMessage({ action: "dev_set_chain", count: 4 }),
      );
      await turnPage.waitForFunction(
        () => {
          const chain = document.getElementById("game-status-chain");
          return (
            chain &&
            !chain.classList.contains("hidden") &&
            /\+4/.test(chain.textContent || "")
          );
        },
        { timeout: 5000 },
      );
      ...
      await turnPage.evaluate(() =>
        sendMessage({ action: "dev_set_chain", count: 0 }),
      );
      await turnPage.waitForFunction(
        () => {
          const chain = document.getElementById("game-status-chain");
  ...
  it(
    "dev panel state info pane shows lobby and game state",
    { timeout: 30000 },
    async () => {
      const ctxA = await browser.newContext();
      const ctxB = await browser.newContext();
  ...
  it(
    "in-game status pills show draw mode and pending chain",
    { timeout: 30000 },
    async () => {
      const ctxA = await browser.newContext();
      const ctxB = await browser.newContext();
  ...
  it(
    "off-turn .not-playable cards have default cursor too",
    { timeout: 30000 },
    async () => {
      const ctxA = await browser.newContext();
      const ctxB = await browser.newContext();
      ...
      await offPage.waitForFunction(
        () => {
          const npCard = document.querySelector(
            "#player-hand .card.not-playable",
          );
          return (
            !!npCard &&
            document.body.classList.contains("player-action-disabled")
          );
        },
        { timeout: 5000 },
      );
  ...
  it(
    ...
    async () => {
      ...
      await targetPage.evaluate(() =>
        sendMessage({ action: "dev_add_cards", count: 4 }),
      );
